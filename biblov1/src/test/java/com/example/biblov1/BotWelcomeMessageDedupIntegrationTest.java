...
@ActiveProfiles("test")
class BotWelcomeMessageDedupIntegrationTest {
    ...
    @Test
    void should_not_send_duplicate_welcome_message_after_repeated_swipes() throws Exception {
        ...
        TestAuthSupport.TestUser human = TestAuthSupport.registerAndLogin(
                ...
                objectMapper,
                "Human Dedup",
                "dedup-human@example.com",
                "password123"
        );

        swipeRight(human.token(), botId);
        ChatRoom chatRoom = waitForChatRoom(human.id(), botId, Duration.ofSeconds(5));
        assertThat(chatRoom).isNotNull();
        Message firstMessage = waitForFirstBotMessage(chatRoom, botId, Duration.ofSeconds(5));
        ...
        long botMessageCount = messageRepository.findByChatRoomOrderByTimestampAsc(chatRoom).stream()
                .filter(message -> message.getSender() != null && botId.equals(message.getSender().getId()))
                .count();
        assertThat(botMessageCount).isEqualTo(1);
    }

    private void swipeRight(String token, Long swipedUserId) throws Exception {
        mockMvc.perform(
    ...
    private Message waitForFirstBotMessage(ChatRoom room, Long botId, Duration timeout) throws InterruptedException {
        Instant deadline = Instant.now().plus(timeout);
        while (Instant.now().isBefore(deadline)) {
            Message fromBot = messageRepository.findByChatRoomOrderByTimestampAsc(room).stream()
                    .filter(message -> message.getSender() != null && botId.equals(message.getSender().getId()))
                    .findFirst()
                    .orElse(null);
            if (fromBot != null) {
