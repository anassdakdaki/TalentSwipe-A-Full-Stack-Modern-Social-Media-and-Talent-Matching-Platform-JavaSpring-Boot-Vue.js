  ...
  methods: {
    async handleRegister() {
      ...
      try {
        ...
        try {
          const testResponse = await axios.get('http://localhost:8080/api/auth/register')
          console.log('Test response:', testResponse)
        } catch (testError) {
          console.log('Test request error:', testError)
        }
        ...
        const response = await axios({
          ...
          validateStatus: null // Don't validate status, we'll handle it manually
        })
